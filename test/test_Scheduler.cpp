...
void test_fiber1() {
     ...
     */
    bokket::Scheduler::getThis()->schedule(bokket::Fiber::getThis());


    BOKKET_LOG_INFO(g_logger) << "before test_fiber1 yield";
    bokket::Fiber::getThis()->yield();
    BOKKET_LOG_INFO(g_logger) << "after test_fiber1 yield";
...
void test_fiber2() {
    ...
    BOKKET_LOG_INFO(g_logger) << "test_fiber2 end";
}

void test_fiber3() {
    BOKKET_LOG_INFO(g_logger) << "test_fiber3 begin";
    BOKKET_LOG_INFO(g_logger) << "test_fiber3 end";
}

void test_fiber5() {
    static int count = 0;

    BOKKET_LOG_INFO(g_logger) << "test_fiber5 begin, i = " << count;
    BOKKET_LOG_INFO(g_logger) << "test_fiber5 end i = " << count;
...
void test_fiber4() {
    ...
    for (int i = 0; i < 3; i++) {
        bokket::Scheduler::getThis()->schedule(test_fiber5, bokket::getThreadId());
    }

    BOKKET_LOG_INFO(g_logger) << "test_fiber4 end";
}
...
int main() {
    ...
    bokket::Fiber::ptr fiber(new bokket::Fiber(&test_fiber3));
    sc.schedule(fiber);
